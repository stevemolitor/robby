...
*** robby-view
Query AI from the region, and respond in a read-only markdown view window. If no selected region read prompt from buffer content. Maintain conversational history of previous prompts and responses, up to ~robby-max-history~ prompt/response pairs.
