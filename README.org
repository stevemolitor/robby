...
You can experiment with the various options to tune a particular command. For
example for certain commands you may want to set the =robby-chat-tempature= to =0=
to produce more deterministic results. For other commands you may want to choose
a different model, higher max tokens ec. See the [[https://platform.openai.com/docs/api-reference/chat/create][OpenAI Chat API Documentation]]
for details on the various options. When you save a command via
~robby-insert-last-command~ the API options you used will be persisted with the
command definition.
