...
Robby passes the conversation history of previous message to OpenAI. The ~robby-max-history~ customization variable specifices the maximum number of previous prompt/response pairs to keep in
the conversation history. Its default is 2. Increasing this value will pass more history context to OpenAI, at the cost of using more tokens. Setting it to 0 to turn conversation history off.
