...
(ert-deftest robby--prepend-response-to-region ()
  (with-temp-buffer
    (insert "region text")
    (robby--prepend-response-to-region "AI response" (point-min) (point-max))
    (should
...
(ert-deftest robby--append-response-after-region ()
  (with-temp-buffer
    (insert "region text")
    (robby--append-response-after-region "AI response" (point-min) (point-max))
    (should
     (equal (buffer-substring-no-properties (point-min) (point-max))
            "region text\nAI response"))))
...
(ert-deftest robby--replace-region-with-response ()
  (with-temp-buffer
    (insert "region text")
    (robby--replace-region-with-response "AI response" (point-min) (point-max))
    (should
     (equal (buffer-substring-no-properties (point-min) (point-max))
            "AI response"))))
